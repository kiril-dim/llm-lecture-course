...
**Инструкции:** Тези упражнения покриват основните концепции от лекцията, включително N-gram езикови модели и perplexity. Решенията са дадени накрая.
...
## Упражнение 11: Bigram вероятности

Даден е малък корпус от 3 изречения (с добавени `<s>` и `</s>` маркери):
- `<s>` the cat sat `</s>`
- `<s>` the cat ate `</s>`
- `<s>` the dog sat `</s>`

**Задача:** Използвайки Maximum Likelihood Estimation (без smoothing):
a) Изчислете $P(\text{cat} | \text{the})$
b) Изчислете $P(\text{sat} | \text{cat})$
c) Изчислете $P(\text{bird} | \text{the})$

Формула: $P(w_i | w_{i-1}) = \frac{C(w_{i-1}, w_i)}{C(w_{i-1})}$

---

## Упражнение 12: Perplexity

Bigram модел дава следните вероятности за изречението "the cat sat":
- $P(\text{the} | \text{<s>}) = 1.0$
- $P(\text{cat} | \text{the}) = 0.5$
- $P(\text{sat} | \text{cat}) = 0.5$
- $P(\text{</s>} | \text{sat}) = 0.5$

**Задача:** Изчислете perplexity на това изречение.

Формула: $PP = \left( \prod_{i=1}^{N} P(w_i | w_{i-1}) \right)^{-1/N}$

където $N$ е броят на предсказаните думи (включително `</s>`).
...
## Решение 11
Броим bigrams в корпуса:
- $C(\text{the}) = 3$ (появява се 3 пъти)
- $C(\text{the}, \text{cat}) = 2$
- $C(\text{the}, \text{dog}) = 1$
- $C(\text{cat}) = 2$
- $C(\text{cat}, \text{sat}) = 1$
...
a) $P(\text{cat} | \text{the}) = \frac{C(\text{the}, \text{cat})}{C(\text{the})} = \frac{2}{3} \approx 0.667$

b) $P(\text{sat} | \text{cat}) = \frac{C(\text{cat}, \text{sat})}{C(\text{cat})} = \frac{1}{2} = 0.5$

c) $P(\text{bird} | \text{the}) = \frac{0}{3} = 0$ (думата "bird" никога не следва "the")

## Решение 12
Вероятност на изречението:
$$P = 1.0 \times 0.5 \times 0.5 \times 0.5 = 0.125$$

Брой предсказани думи: $N = 4$ (the, cat, sat, `</s>`)

Perplexity:
$$PP = (0.125)^{-1/4} = \left(\frac{1}{0.125}\right)^{1/4} = 8^{0.25} = \sqrt[4]{8} \approx 1.68$$

Интерпретация: Моделът е "изненадан" толкова, колкото ако избираше между ~1.68 равновероятни думи на всяка стъпка.
